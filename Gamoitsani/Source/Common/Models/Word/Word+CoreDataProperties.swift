...
extension Word {
    ...
    @NSManaged public var isGeorgianOrigin: Bool
    @NSManaged public var formalityLevel: Int16
    @NSManaged public var isProperNoun: Bool
    @NSManaged public var wordType: String?
    @NSManaged public var isAbstract: Bool
    @NSManaged public var ageAppropriateness: String?
    @NSManaged public var relatedWordsData: Data?
    ...
    var relatedWords: [String] {
        get {
            guard let data = relatedWordsData else { return [] }
            return (try? NSKeyedUnarchiver.unarchivedObject(ofClass: NSArray.self, from: data)) as? [String] ?? []
        }
        set {
            relatedWordsData = try? NSKeyedArchiver.archivedData(withRootObject: newValue, requiringSecureCoding: true)
        }
    }
}
